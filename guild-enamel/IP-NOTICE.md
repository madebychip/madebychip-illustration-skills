...
The workflow, prompt templates, and instructions in this repository (`SKILL.md`, `references/`) are provided under MIT — reuse, adapt, and republish freely.

The specific visual designs in `references/` and `assets/` — the exact enamel color combination, logo mark, and facet motifs shown in the example images — are not covered by that license. If you're forking this for your own brand, generate your own reference set using your own values from the "Brand tokens" section of `SKILL.md`, rather than reusing these images directly.
